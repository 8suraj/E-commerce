...
export default async function userLoginValidator(
	...
	if (!password) {
		errors.body.push(
			'password property in user can&apos;t be empty'
		);
	} else if (typeof password !== 'string') {
		errors.body.push(
			'password property in user must be a string'
		);
	}

	if (!email) {
		errors.body.push(
			'email property in user can&apos;t be empty'
		);
	} else if (typeof email !== 'string') {
		errors.body.push(
			'email property in user must be a string'
		);
	}
